...
----
当时间和/或区域字段未设置时，它们会被省略.When the time and/or zone fields are unset, they are omitted. For negative dates, the `/` separator is used instead of `-` for better readability.
